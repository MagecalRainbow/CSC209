...
int accept_connection(int fd, struct sockname *usernames) {
    ...
    int bytes_read = read(client_fd, username, BUF_SIZE);
    username[bytes_read - 1] = '\0';

    usernames[user_index].sock_fd = client_fd;
    usernames[user_index].username = malloc(sizeof(username));
    strcpy(usernames[user_index].username, username);
    return client_fd;
}
 ...
 */
int read_from(int client_index, struct sockname *usernames, char *new_buf) {
...
int main(void) {
    ...
    while (1) {
        ...
        char *new_buf = malloc((BUF_SIZE*2 + 1)*sizeof(char));
        ...
        for (int index = 0; index < MAX_CONNECTIONS; index++) {
            if (usernames[index].sock_fd > -1 && FD_ISSET(usernames[index].sock_fd, &listen_fds)) {
                ...
                int client_closed = read_from(index, usernames, new_buf);
                char *ptr = strtok(new_buf, ": ");
                ptr = strtok(NULL, ":\n");
                if (client_closed > 0) {
                    FD_CLR(client_closed, &all_fds);
                    printf("Client %d disconnected\n", client_closed);
                } else {
                    printf("%s:%s\n", usernames[index].username, ptr);
                }
            }
        }
    }
