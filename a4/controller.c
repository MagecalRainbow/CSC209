...
int process_message(struct cignal *cig, int *device_record) {
    //The cignal message is not valid.
    if (is_valid_type(cig) == -1) {
        fprintf(stderr, "Received corrupted cignal! The message is discarded...\
                \n");
        return -1;
    } 
   //The message cignal is valid. 
    else {
        //The message cignal is registered.
        if (is_registered(cig->hdr.device_id, device_record) == 1) {
            adjust_fan(cig);
            //The message cignal coming from a humidity sensor.
            if (cig->hdr.device_type == HUMIDITY) {
                printf("Humidity: %.4f --> Device_ID: %d\n", cig->value, 
                        cig->hdr.device_id);
            }
            //The message cignal coming from a temperature sensor.
        ...
        //The device in cignal message is not registered. 
        //Thus, register the device. 
        else {
